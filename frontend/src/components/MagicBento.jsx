...
const MagicBento = ({
  ...
  const handleDelete = async (id) => {
    try {
      ...
      setPipelines(updated); // You must receive setPipelines from props
      } catch (err) {
        alert("Failed to delete pipeline");
        console.error(err);
      }
    };
  const cardData = (pipelines || [])
  .filter((p) => p && typeof p === "object")
  .map((pipeline) => ({
    ...pipeline,
    label: pipeline.label || "Untitled Pipeline",
      ...
      <BentoCardGrid gridRef={gridRef}>
        ...
        {cardData.map((card, index) => {
          ...
          const cardProps = {
            ...
            style: {
              backgroundColor: card.color || "#1f1f1f",
            ...
            return (
              ...
              >
                <div className="card__header">
                  ...
                  <button
                    onClick={() => {
                      const confirmed = window.confirm(`Delete pipeline "${card.name}"?`);
                      if (confirmed) handleDelete(card.id);
                    }}
                    ...
                    <span
                      className={`inline-block px-2 py-0.5 text-xs font-medium rounded-full ${
                        card.status?.toLowerCase() === "running"
                          ...
                          : card.status?.toLowerCase() === "paused"
